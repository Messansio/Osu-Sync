...
void OsuPathUtility::askStableOrLazer() {
    ...
    osuType = temp[0] ? std::stoi(temp) : 0;
}
...
bool OsuPathUtility::locateOsuFolder() {
    ...
    if (osuType == 2 || osuType == 0) {
        error = locateOsuSFolder();
        if (error) return error;
        else std::cout << "Found osu!stable directory at " << osuSPath.generic_string() << "\n";
    }

    if (osuType == 2 || osuType == 1) {
        error = locateOsuLFolder();
        if (error) return error;
        else std::cout << "Found osu!lazer directory at " << osuSPath.generic_string() << "\n";
    }
...
void OsuPathUtility::createDirectory(const std::filesystem::path& directory) const {
